...
По умолчанию Boost подключается через `find_package` (требуется установленный `libboost-system-dev`). Если Boost не установлен в системе или хочется зафиксировать версию, можно собрать с опцией `USE_FETCH_BOOST=ON` — тогда Boost будет скачан через `FetchContent` :
...
Поток данных:
1. КлиентА генерирует пакет (`AccelService` - синус, интервал 20 мс) и шлёт на сервер.
2. Сервер читает пакет, отбрасывает подряд идущий дубликат (`eps = 1e-4` по `x`/`y`/`z`) и пересылает на КлиентБ.
...
- **`TcpConnection`** - RAII над сокетом, асинхронные построчные чтение/запись.
